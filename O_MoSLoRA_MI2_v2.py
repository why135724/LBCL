...
def merge_adapters(model):
    ...
    W <- W + sum_t B @ M2^T @ M1^T @ A   (equivalent to forward computation)
    """
    with torch.no_grad():
        for name, module in model.named_modules():
            if isinstance(module, LoraLinear):
                ...
                for t in range(len(module.lora_A_list)):
                    ...
                    M1 = module.lora_AB_list[t]    # [r, r]
                    M2 = module.lora_AB1_list[t]   # [r, r]

                    # Fix: use A instead of A.T
...
train_df = pd.read_json(os.path.join(base_dir, "data", "new_MI2_train.jsonl"), lines=True)
print('train_df.len', len(train_df))

test_df = pd.read_json(os.path.join(base_dir, "data", "new_MI2_test.jsonl"), lines=True)
