...
def orthogonal_loss(A_history: torch.Tensor, A_current: torch.Tensor) -> torch.Tensor:
    ...
    A_current: [r, d]
    """
    A_history = F.normalize(A_history, p=2, dim=1, eps=1e-8)
    A_current = F.normalize(A_current, p=2, dim=1, eps=1e-8)
...
def replace_linear_with_lora(
    ...
    for name, child in module.named_children():
        if any(s in name for s in ['embed', 'norm', 'lm_head']):
            ...
            setattr(module, name, LoraLinear(child, r, alpha, dropout_p, num_mixers))
        else:
...
def merge_adapters(model):
    ...
    W <- W + sum_t B @ M_k @ ... @ M_1 @ A   (equivalent to forward computation)
    """
    with torch.no_grad():
        for name, module in model.named_modules():
            if isinstance(module, LoraLinear):
...
paths = {
    "MI2": (
        os.path.join(base_dir, "data", "MI2_train.jsonl"),
        os.path.join(base_dir, "data", "MI2_test.jsonl"),
        f"new_MI2_train_{RUN_TAG}.jsonl",
        f"new_MI2_test_{RUN_TAG}.jsonl"
    ),
    "MI1": (
    ...
    "ERN": (
        os.path.join(base_dir, "data", "ERN_train.jsonl"),
        os.path.join(base_dir, "data", "ERN_test.jsonl"),
        f"new_ERN_train_{RUN_TAG}.jsonl",
        f"new_ERN_test_{RUN_TAG}.jsonl"
    )
}
...
train_df = pd.read_json(os.path.join(base_dir, "data", f"new_MI2_train_{RUN_TAG}.jsonl"), lines=True)
print('train_df.len', len(train_df))

test_df = pd.read_json(os.path.join(base_dir, "data", f"new_MI2_test_{RUN_TAG}.jsonl"), lines=True)
...
replace_linear_with_lora(model, r=8, alpha=32, dropout_p=0.1, num_mixers=NUM_MIXERS)
model.to(device)
...
for subject_id in range(9):
    print(f"\n{'='*40}")
    print(f"Training Subject {subject_id + 1}/9")
    print(f"{'='*40}")
