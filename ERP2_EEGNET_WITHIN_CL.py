...
def EA(x):
        ...
        for i in range(x.shape[0]):
                cov[i] = np.cov(x[i])
        refEA = np.mean(cov, 0)
        sqrtRefEA=fractional_matrix_power(refEA, -0.5)+ (0.00000001)*np.eye(56)
...
class PatchEmbedding(nn.Module):
    def __init__(self, emb_size=40):
        ...
        self.shallownet = nn.Sequential(
            ...
            nn.BatchNorm2d(40),
            nn.ELU(),
            nn.AvgPool2d((1, 75), (1, 15)),  # Pooling, slicing along temporal dimension to obtain "patches"
            nn.Dropout(0.5),
        )
        self.projection = nn.Sequential(
            nn.Conv2d(40, emb_size, (1, 1), stride=(1, 1)),  # Adjust channel dimension
            Rearrange('b e (h) (w) -> b (h w) e'),  # Rearrange to sequence format
        )
...
# 6. Transformer encoder (multi-layer stack)
...
class ClassificationHead(nn.Sequential):
    def __init__(self, emb_size, n_classes):
        ...
        self.fc = nn.Sequential(
            nn.Linear(15400, 256),  # needs to be adjusted
            nn.ELU(),
            nn.Dropout(0.5),
            nn.Linear(256, 32),
            nn.ELU(),
    ...
    def forward(self, x):
        ...
        #print('x',x.shape)
        out = self.fc(x)  # Pass through fully connected layer for classification
...
if model_name == 'DeepConvNet':
    model = DeepConvNet(2, 56, 260)  # 750 is the timepoint length
elif model_name == 'ShallowConvNet':
    model = ShallowConvNet(2, 56, 24024)  # 9152 needs to be calculated based on actual data
elif model_name == 'EEGNet':
    model = EEGNet(in_chan=56, fc_num=128, out_chann=2)
    ...
    args = Args()
    args.chn = 56  # 22 channels
    args.time_sample_num = 260  # 1000 timepoints
...
if __name__ == "__main__":
    ...
    print(f"\nAverage within-subject test accuracy: {mean_accuracy:.2f}% ± {std_accuracy:.2f}%")
    ...
    print(f"\nAverage within-subject test accuracy (continual learning): {mean_accuracy:.2f}% ± {std_accuracy:.2f}%")
