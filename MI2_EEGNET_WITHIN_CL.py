...
# # CUDA_VISIBLE_DEVICES=2 python train_linjiu_diy.py
...
# CUDA_VISIBLE_DEVICES=5 python -u MI_dataset.py
...
def compute_quality_scores(X, y, n_clusters=6):
    """
    Compute quality scores for data samples, considering representativeness, confidence, and outlier detection.

    Parameters:
    ----------
    X : array-like, feature matrix
    y : array-like, label vector
    n_clusters : int, number of clusters, default=6

    Returns:
    ----------
    scores : array, comprehensive quality score for each sample
    """
    # 1. Data standardization
    scaler = StandardScaler()
    X_scaled = scaler.fit_transform(X)

    # 2. L2 normalization (convert vectors to unit vectors, Euclidean distance equivalent to cosine distance)
    normalizer = Normalizer(norm='l2')
    X_normalized = normalizer.fit_transform(X_scaled)

    # 3. Clustering analysis - compute representativeness scores
    representativeness_scores = _compute_representativeness_scores(
        X_normalized, n_clusters
    )

    # 4. Classification confidence - compute confidence scores
    confidence_scores = _compute_confidence_scores(X_scaled, y)

    # 5. Outlier detection - compute outlier scores
    outlier_scores = _compute_outlier_scores(X_scaled)

    # 6. Combined scoring (currently using confidence scores only)
    # Weight configuration: confidence: 1.0, representativeness: 0.0, outlier: 0.0
...
def _compute_representativeness_scores(X_normalized, n_clusters):
    """Compute representativeness scores for samples (based on distance to cluster centers)"""
    kmeans = KMeans(n_clusters=n_clusters, random_state=42)
    kmeans.fit(X_normalized)

    # Compute cosine distance to cluster centers
    cluster_centers = kmeans.cluster_centers_
    cosine_similarities = cosine_similarity(X_normalized, cluster_centers)

    # Convert to cosine distance (1 - similarity), take minimum distance
    min_cosine_distances = (1 - cosine_similarities).min(axis=1)

    # Convert to score: smaller distance -> higher score
    representativeness_scores = 1 / (1 + min_cosine_distances)

    return representativeness_scores


def _compute_confidence_scores(X_scaled, y):
    """Compute classification confidence scores"""
    classifier = RandomForestClassifier(n_estimators=100, random_state=42)
    classifier.fit(X_scaled, y)

    probabilities = classifier.predict_proba(X_scaled)

    # Get probability corresponding to true label as confidence score
    confidence_scores = probabilities[np.arange(len(y)), y]

    return confidence_scores


def _compute_outlier_scores(X_scaled, contamination=0.1):
    """Compute outlier detection scores"""
    ...
    # Transform predictions to [0, 1] range (-1,1 -> 0,1)
    outlier_scores = (outlier_predictions + 1) / 2

    return outlier_scores


def _combine_scores(confidence_scores, rep_scores, outlier_scores, weights):
    """Combine different scores into comprehensive quality scores"""
    weighted_scores = (
            weights[0] * confidence_scores +
            weights[1] * rep_scores +
            weights[2] * outlier_scores
    )

    return weighted_scores


def select_samples_by_quality(y_train, quality_scores, samples_per_class):
    """Select samples for each class based on quality scores"""
    selected_indices = []
    ...
    for label in np.unique(y_train):
        ...
        # Get quality scores for current class
        label_scores = quality_scores[label_indices]

        # Sort by score in descending order
        sorted_indices = np.argsort(label_scores)[::-1]

        # Determine number of samples to select
        k = min(samples_per_class[label], len(sorted_indices))

        # Select top-k samples with highest scores
        selected_indices.extend(label_indices[sorted_indices[:k]])
...
def select_samples_by_random(y_train, quality_scores, samples_per_class):
    """Select samples for each class based on quality scores (random selection)"""
    selected_indices = []
    ...
    for label in np.unique(y_train):
        ...
        # Use random permutation instead
        # Create random indices
        sorted_indices = np.random.permutation(len(label_indices))

        # Determine number of samples to select
        k = min(samples_per_class[label], len(sorted_indices))

        # Select first k samples after random permutation
        selected_indices.extend(label_indices[sorted_indices[:k]])
...
def EA(x):
        cov = np.zeros((x.shape[0],22, 22))
        for i in range(x.shape[0]):
                cov[i] = np.cov(x[i])
        refEA = np.mean(cov, 0)
        sqrtRefEA=fractional_matrix_power(refEA, -0.5)+ (0.00000001)*np.eye(22)
...
# 1. Convolution module (for extracting local features)
class PatchEmbedding(nn.Module):
    def __init__(self, emb_size=40):
        ...
        self.shallownet = nn.Sequential(
            ...
            nn.BatchNorm2d(40),
            nn.ELU(),
            nn.AvgPool2d((1, 75), (1, 15)),  # Pooling, slice along time dimension to get "patches"
            nn.Dropout(0.5),
        )
...
class ClassificationHead(nn.Sequential):
    def __init__(self, emb_size, n_classes):
        ...
        self.fc = nn.Sequential(
            nn.Linear(440, 256),  # needs adjustment
            nn.ELU(),
            nn.Dropout(0.5),
            nn.Linear(256, 32),
            nn.ELU(),
...
def load_all_subjects_data():
    ...
    for i in range(1, 10):
        # Load .mat file
        data = loadmat(f'MI2/A{i}.mat')
        x = data['x']  # EEG data (22 channels x 750 timepoints x 144 trials)
        y = data['y'].flatten()  # Labels (144,)

        # Transpose data from (channels, time, trials) to (trials, channels, time)
...
def prepare_within_subject_data(all_subjects_data, test_subject_idx):
    ...
    for idx, subject_data in enumerate(all_subjects_data):
        ...
        if idx == test_subject_idx:
            ...
            X_train = x[:int(0.6 * n_trials), :, :]  # (n_trials, 22, 750)
            y_train = y[:int(0.6 * n_trials)]
            ...
            if random_use:
                ...
                # If only need to select samples based on quality scores, then train CSP

                # 1. Split into training and testing sets
                X_train = x[:split_idx, :, :]
                y_train = y[:split_idx]

                # 2. First train CSP with all training data
                csp = CSP(n_components=10, reg=None, log=True, norm_trace=False)
                X_train_csp = csp.fit_transform(X_train, y_train)

                # 3. Standardization and quality scoring
                scaler = StandardScaler()
                X_train_scaled = scaler.fit_transform(X_train_csp)
                quality_scores = compute_quality_scores(X_train_scaled, y_train)

                # 4. Select high-quality samples
                SAMPLES_PER_CLASS = {0: int(0.3 * n_trials), 1: int(0.3 * n_trials)}
                selected_indices = select_samples_by_quality(y_train, quality_scores, SAMPLES_PER_CLASS)

                # 5. Retrain model with selected samples
    ...
    X_augmented,y_augmented = sliding_window_augmentation(X_train,y_train, window_length=250, step_size=600)
...
if model_name == 'DeepConvNet':
    model = DeepConvNet(2, 22, 250)  # 750 is timepoint length
elif model_name == 'ShallowConvNet':
    model = ShallowConvNet(2, 22, 9152)  # 750 corresponds to 29458, 250 corresponds to 9152
elif model_name == 'EEGNet':
    model = EEGNet(in_chan=22, fc_num=120, out_chann=2)  # 750 corresponds to 376, 250 corresponds to 120
    ...
    args = Args()
    args.chn = 22  # 22 channels
    args.time_sample_num = 250  # 1000 timepoints
    args.patch_size = 125  # 125 points as one patch
    args.class_num = 2  # 4-class classification task
    # Model instantiation
    model = DBConformer(
        args,
...
if __name__ == "__main__":
    ...
    print(f"Successfully loaded {num_subjects} subjects' data")
    ...
    for test_idx in range(num_subjects):
        print(f"\n{'=' * 50}")
        print(f"Performing cross-validation round {test_idx + 1}/{num_subjects}")
        ...
        # Initialize model
        # Note: fc_num needs to be calculated based on input data shape, using previous value 376
    ...
    # Print accuracy for each subject during continuous learning process
    ...
    print(f"\nWithin-subject test average accuracy_Continuous Learning: {mean_accuracy:.2f}% ± {std_accuracy:.2f}%")

    # BWT calculation


    # Calculate difference between corresponding positions
