...
def compute_quality_scores(X, y, n_clusters=6):
    """
    Compute quality scores for data samples, considering representativeness,
    ...
    # 3. Cluster analysis - compute representativeness scores
    ...
    # 6. Combined scoring (currently using only confidence scores)
...
def _compute_representativeness_scores(X_normalized, n_clusters):
    ...
    # Convert to score: smaller distance yields higher score
...
def _compute_outlier_scores(X_scaled, contamination=0.1):
    ...
    # Convert predictions to [0, 1] range (-1,1 -> 0,1)
...
def _combine_scores(confidence_scores, rep_scores, outlier_scores, weights):
    """Combine different scores into a comprehensive quality score"""
    weighted_scores = (
...
def select_samples_by_random(y_train, quality_scores, samples_per_class):
    ...
    for label in np.unique(y_train):
        ...
        # Use random permutation instead of sorting
...
class PatchEmbedding(nn.Module):
    def __init__(self, emb_size=40):
        ...
        self.shallownet = nn.Sequential(
            ...
            nn.BatchNorm2d(40),
            nn.ELU(),
            nn.AvgPool2d((1, 75), (1, 15)),  # Pooling, slice along temporal dimension to obtain "patches"
            nn.Dropout(0.5),
        )
...
# 6. Transformer encoder (stacked layers)
...
class ClassificationHead(nn.Sequential):
    def __init__(self, emb_size, n_classes):
        ...
        self.fc = nn.Sequential(
            nn.Linear(16720, 256),  # needs adjustment
            nn.ELU(),
            nn.Dropout(0.5),
            nn.Linear(256, 32),
            nn.ELU(),
    ...
    def forward(self, x):
        ...
        return out  # Return features and classification result
...
def load_all_subjects_data():
    ...
    for i in range(1, 8):
        # Load .mat file
        data = loadmat(f'MI1/A{i}.mat')
        x = data['x']  # EEG data (22 channels x 750 timepoints x 144 trials)
        y = data['y'].flatten()  # Labels (144,)

        # Transform data from (channels, time, trials) to (trials, channels, time)
...
def sliding_window_augmentation(X,y, window_length=250, step_size=10):
    ...
    for trial in range(n_trials):
        start_idx = 0
        # Calculate number of windows that can be extracted for current trial
...
def prepare_within_subject_data(all_subjects_data, test_subject_idx):
    ...
    for idx, subject_data in enumerate(all_subjects_data):
        ...
        if idx == test_subject_idx:
            ...
            if random_use:
                ...
                # 2. Train CSP with all training data first
    ...
    X_augmented, y_augmented = sliding_window_augmentation(X_train, y_train, window_length=250, step_size=300)
    ...
    # Original shape: (trials, channels, timepoints)
    # Target shape: (trials, 1, timepoints, channels)
...
if model_name == 'DeepConvNet':
    model = DeepConvNet(2, 59, 250)  # 750 is timepoint length
elif model_name == 'ShallowConvNet':
    model = ShallowConvNet(2, 59, 24544)  # 750 corresponds to 29458, 250 corresponds to 9152
...
if __name__ == "__main__":
    ...
    for test_idx in range(num_subjects):
        print(f"\n{'=' * 50}")
        print(f"Performing cross-validation {test_idx + 1}/{num_subjects}")
    ...
    # Calculate differences at corresponding positions
