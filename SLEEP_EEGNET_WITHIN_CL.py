...
class PatchEmbedding(nn.Module):
    def __init__(self, emb_size=40):
        ...
        self.projection = nn.Sequential(
            nn.Conv2d(40, emb_size, (1, 1), stride=(1, 1)),  # Adjust channel number
            Rearrange('b e (h) (w) -> b (h w) e'),  # Rearrange into sequence form
        )
...
class ClassificationHead(nn.Sequential):
    def __init__(self, emb_size, n_classes):
        ...
        self.fc = nn.Sequential(
            nn.Linear(1680, 256),  # needs to be adjusted
            nn.ELU(),
            nn.Dropout(0.5),
            nn.Linear(256, 32),
            nn.ELU(),
...
def compute_bca(y_true, y_pred):
    """
    Compute Balanced Classification Accuracy (BCA), handling potential division by zero
    """
    ...
    # Method 1: Use mask to avoid division by zero
    mask = row_sums > 0
    per_class_acc = np.zeros(cm.shape[0])
    per_class_acc[mask] = cm.diagonal()[mask] / row_sums[mask]

    # For classes with no samples in test set:
    # 1. Exclude these classes from average calculation
    # 2. Set their accuracy to 0
    # Using option 2 here
    per_class_acc[~mask] = 0.0
    ...
    if np.any(row_sums == 0):
        zero_classes = np.where(row_sums == 0)[0]
        print(f"Warning: The following classes have no samples in test set: {zero_classes}")
        print(f"These classes' accuracy will be set to 0")
...
def test_model(model_name, model, test_loader):
    ...
    # Compute Balanced Classification Accuracy (BCA)
    bca = compute_bca(y_true, y_pred)
...
def train_model(model_name, model, train_loader, criterion, optimizer, epochs=1):
    ...
    for epoch in range(epochs):
        ...
        #accuracy = test_model(model_name, model, train_loader)
...
def load_all_subjects_data():
    ...
    for i in range(1, 11):
        # Load .mat file
        data = loadmat(f'SLEEP1/A{i}.mat')
        x = data['X']  # EEG data (trials, channels, time)
        y = data['y'].flatten()  # Labels (XXX,)
...
def sliding_window_augmentation(X, y, window_length=250, step_size=10):
    ...
    # Concatenate all windows along dimension 0
...
def prepare_within_subject_data(all_subjects_data, test_subject_idx):
    """
    Prepare within-subject training and test data
    ...
    for idx, subject_data in enumerate(all_subjects_data):
        ...
        if idx == test_subject_idx:
            ...
            for label, count in zip(unique_labels, counts):
                if count == 1:
                    # Find index of this sample
                    label_idx = np.where(y == label)[0][0]
                    # Duplicate sample
                    x = np.concatenate([x, np.expand_dims(x[label_idx], axis=0)], axis=0)
            ...
            # Keep half of label 4 data in X_train
            label_4_indices = np.where(y_train == 4)[0]
            if len(label_4_indices) > 0:
                # Randomly select half of indices
                np.random.seed(42)
                n_keep = len(label_4_indices) // 1  # 8,16
                keep_indices = np.random.choice(label_4_indices, n_keep, replace=False)
                remove_indices = np.setdiff1d(label_4_indices, keep_indices)

                # Keep non-label-4 data and half of label-4 data
    ...
    X_augmented, y_augmented = sliding_window_augmentation(X_train, y_train, window_length=3000, step_size=2)
...
if model_name == 'DeepConvNet':
    model = DeepConvNet(6, 2, 3000)  # 750 is timepoint length
elif model_name == 'ShallowConvNet':
    model = ShallowConvNet(6, 2, 11024)  # 9152 needs to be calculated empirically
elif model_name == 'EEGNet':
    model = EEGNet(in_chan=2, fc_num=1496, out_chann=6)  # 750 corresponds to 376, 250 corresponds to 120
elif model_name == 'EEGConformer':
    model = Conformer(emb_size=40, depth=6, n_classes=6)  # need to adjust fully connected layer definition in model definition
elif model_name == 'DBConformer':
    args = Args()
    args.chn = 2  # 22 channels
    args.time_sample_num = 3000  # 1000 timepoints
    args.patch_size = 125  # 125 points as one patch
    args.class_num = 6  # 4-class task
    # Model instantiation
    model = DBConformer(
        args,
...
# class_weights = torch.tensor([5.0, 5.0 ,5.0,5.0,1.0,5.0])  # Assuming classes 0 and 1, class 1 is minority class
...
if __name__ == "__main__":
    ...
    for test_idx in range(num_subjects):
        print(f"\n{'=' * 50}")
        print(f"Running cross-validation iteration {test_idx + 1}/{num_subjects}")
