...
def euclidean_alignment(x):
    ...
    sqrt_ref_ea = fractional_matrix_power(ref_ea, -0.5) + (0.00000001) * np.eye(22)
    x_ea = np.zeros(x.shape)
    for i in range(x.shape[0]):
...
def load_all_subjects_data():
    ...
    for i in range(1, 10):
        # Load .mat file
        data = loadmat(f'/data2/hywu/LLaMA-Factory/MI2/A{i}.mat')
...
def sliding_window_augmentation(X, y, window_length=250, step_size=10):
    ...
    for trial in range(n_trials):
        start_idx = 0
        # Calculate number of windows for current trial
...
def prepare_within_subject_data(all_subjects_data, test_subject_idx):
    ...
    X_augmented, y_augmented = sliding_window_augmentation(
        X_train, y_train, window_length=200, step_size=800
    )
    ...
    # Transpose data to match EEGNet input requirements
    # From: (trials, channels, timepoints)
    # To:   (trials, 1, timepoints, channels)
    X_train = X_train.transpose(0, 2, 1)       # (n_trials, 200, 22)
    X_train = np.expand_dims(X_train, axis=1)  # (n_trials, 1, 200, 22)

    X_test = X_test.transpose(0, 2, 1)         # (n_trials, 200, 22)
    X_test = np.expand_dims(X_test, axis=1)    # (n_trials, 1, 200, 22)
...
if model_name == 'EEGNet':
    model = EEGNet(in_chan=22, fc_num=120, out_chann=2)  # 750->376, 250->120
elif model_name == 'CBraMod':  # Modify settings according to dataset
    device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
...
if __name__ == "__main__":
    ...
    for test_idx in range(num_subjects):
        ...
        print(f"Train set shape: {X_train.shape}, Label shape: {y_train.shape}")
        ...
        # Train model (fewer epochs for faster cross-validation)
        print("Starting model training...")
        train_model(model_name, model, classifier, train_loader,
        ...
        print(f"Subject A{test_idx + 1} test accuracy: {accuracy * 100:.2f}%")
    ...
    print(f"\nWithin-subject test mean accuracy: {mean_accuracy:.2f}% +/- {std_accuracy:.2f}%")
    ...
    print(f"\nWithin-subject test mean accuracy (continual learning): {mean_accuracy:.2f}% +/- {std_accuracy:.2f}%")

    # BWT calculation
    # Calculate pairwise differences
    differences = [a - b for a, b in zip(subject_accuracies_cl, subject_accuracies)]

    # Calculate average difference (BWT)
